		...
		<app-card :class-list="['mb-30']">
			<template #header>
				<div class="row align-items-center">
					...
					>
						<project-list-header :checkboxes="checkboxes" @remove="removeAll" />
					</div>

					<div id="filter" class="col-xxl-auto d-xl-block collapse">
			...
			store.getters['project/projects']
				...
				.filter((request) => {
					if (filter.value.type?.value) {
						return (
							filter.value.type.value === request.type ||
							filter.value.type.value === 'all'
						)
					}
