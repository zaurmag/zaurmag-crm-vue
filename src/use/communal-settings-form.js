...
export function useCommunalSettingsForm(initialValues, emit) {
	...
	const {
		...
		'electr',
		yup.number().required('Введите тариф электро счетчика')
	)
	...
	const {
		...
		'trash',
		yup.number().required('Введите тариф на мусор за одного чел.')
	)
	...
	const {
		...
		'people',
		yup.number().required('Введите количество прописанных людей в доме')
	)
